...
function checkArray(obj) {
  if (typeof obj !== null || typeof obj !== null) {
...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    console.log("ERROR: не передано жодного елементу");
  }
  return arr;
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    indexArray.push(arr.indexOf(element));
    indexArray.push(arr.lastIndexOf(element));
  }
  return indexArray;
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (0 <= target < arr.length && 0 <= start < arr.length && 0 <= end < arr.length) {
    arr.copyWithin(target, start, end);
  } else console.log("Неприпустимі індекси");
  return arr;
}
...
function customUnshift(arr, ...elements) {
  ...
  if (!Array.isArray(arr)) {
    ...
    // працює без циклу for, а з ним працює не правильно.
...
function customIncludes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    const isIncludes = arr.includes(element);
    console.log(arr.filter((el) => el === element).length);
    return isIncludes;
  }
}
